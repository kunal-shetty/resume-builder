...
import ResumePreview from "../build-resume/components/ResumePreview"
import ProjectEditor from "../build-resume/components/ProjectEditor"
import ThemeCustomizer from "../build-resume/components/ThemeCustomizer"
import SoundEffects from "../build-resume/components/SoundEffects"
...
export default function BuilderPage() {
  ...
  const handleProfilePictureUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        updateResumeData("profilePicture", result)
        toast({
  ...
  const removeProfilePicture = () => {
    updateResumeData("profilePicture", "")
    toast({
  ...
  const deleteProject = (projectId: string) => {
    const updatedProjects = ResumeData.projects.filter((project) => project.id !== projectId)
    updateResumeData("projects", updatedProjects)
  ...
  const deleteSkill = (skillId: string) => {
    const updatedSkills = ResumeData.skills.filter((skill) => skill.id !== skillId)
    updateResumeData("skills", updatedSkills)
  ...
  const generateResumeFiles = (data: ResumeData) => {
    const indexHtml = `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>${data.name || "Resume"} - ${data.title || "Developer"}</title>
...
<body>
    ...
    return {
      ...
      "package.json": `{
  "name": "${data.name?.toLowerCase().replace(/\s+/g, "-") || "Resume"}",
  "version": "1.0.0",
  "description": "Resume website for ${data.name || "Developer"}",
  "main": "index.html"
}`,
      "README.md": `# ${data.name || "Resume"} Website\n\nBuilt with Resume Builder`,
    }
  }
  ...
  const exportResume = async () => {
    ...
    try {
      ...
      const files = generateResumeFiles(ResumeData)
      ...
      a.href = url
      a.download = `${ResumeData.name?.toLowerCase().replace(/\s+/g, "-") || "Resume"}.zip`
      ...
      toast({
        ...
        description: "Your Resume has been exported as a ZIP file.",
      })
    } catch (error) {
  ...
  const generateAndViewResume = async () => {
    setIsDeploying(true)
    try {
      // Generate Resume files
      const files = generateResumeFiles(ResumeData)
      ...
      setGeneratedResumeUrl(htmlUrl)

      // Create a display URL for the user
      const timestamp = Date.now()
      const slug = customUrl || ResumeData.name?.toLowerCase().replace(/\s+/g, "-") || "Resume"
      ...
      toast({
        ...
        description: "Your Resume is ready to view. Opening in new tab...",
      })

      // Auto-open the Resume in a new tab
      setTimeout(() => {
        const newWindow = window.open(htmlUrl, "_blank")
        if (!newWindow) {
          toast({
            title: "Popup Blocked",
            description: "Please allow popups and try again, or click 'View Resume' button.",
            variant: "destructive",
          })
        }
      }, 500)
  ...
  const viewGeneratedResume = () => {
    if (generatedResumeUrl) {
      const newWindow = window.open(generatedResumeUrl, "_blank")
      if (!newWindow) {
        toast({
          title: "Popup Blocked",
  ...
  const copyDeploymentUrl = async () => {
    if (deploymentUrl) {
      try {
        ...
        toast({
          title: "URL Copied",
          description: "Resume URL has been copied to clipboard.",
        })
      } catch (error) {
        toast({
          title: "Copy Failed",
  ...
  const togglePreviewMode = () => {
    ...
    toast({
      ...
      description: isPreviewMode ? "You can now edit your Resume." : "Viewing your Resume preview.",
    })
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-violet-900">
      {typeof window !== "undefined" && <SoundEffects enabled={ResumeData.soundEffects} />}
      ...
      >
        <div className="max-w-7xl mx-auto px-2 sm:px-4 py-3 sm:py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-white truncate">Resume Builder</h1>
      ...
      {deploymentUrl && (
        ...
        >
          ...
          >
            <h3 className="text-lg font-semibold mb-4 text-green-400">🎉 Resume Generated!</h3>

            <div className="space-y-4">
              <div className="p-4 bg-green-900/20 border border-green-500/20 rounded-lg">
                <p className="text-sm text-gray-300 mb-2">Your Resume is ready to view:</p>
                <p className="text-green-400 font-mono text-sm break-all">{deploymentUrl}</p>
              </div>

              <div className="flex gap-2">
                <Button onClick={viewGeneratedResume} className="flex-1 bg-green-600 hover:bg-green-700">
              ...
              <div className="text-xs text-gray-400">
                ...
                <ul className="list-disc list-inside mt-1 space-y-1">
                  ...
                  <li>Your Resume will be live on the internet!</li>
                </ul>
              </div>
                      ...
                      <TabsContent value="basic" className="space-y-6 mt-6">
                          ...
                          <div className="flex flex-col sm:flex-row items-center gap-4">
                            <div className="relative">
                              <div
                                ...
                                style={{
                                  background: ResumeData.profilePicture
                                    ? "transparent"
                                    : `linear-gradient(45deg, ${ResumeData.theme.primaryColor}, ${ResumeData.theme.secondaryColor})`,
                                }}
                              >
                        ...
                        <div>
                          ...
                          <Textarea
                            ...
                            onChange={(e) => updateResumeData("bio", e.target.value)}
                      ...
                      <TabsContent value="settings" className="space-y-6 mt-6">
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            ...
                            <Switch
                              id="animations"
                              checked={ResumeData.animations}
                              onCheckedChange={(checked) => updateResumeData("animations", checked)}
                            />
                          </div>
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <Switch
                              id="sound"
                              checked={ResumeData.soundEffects}
                              onCheckedChange={(checked) => updateResumeData("soundEffects", checked)}
                            />
                          </div>
                        </div>
                      </TabsContent>
                ...
                <Card className="bg-black/20 backdrop-blur-md border-white/10 text-white sticky top-24">
                  ...
                  <CardContent>
                    <div className="aspect-[9/16] bg-black/30 rounded-lg overflow-hidden">
                      <ResumePreview data={ResumeData} isCompact />
                    </div>
                  </CardContent>
                </Card>
              </div>
